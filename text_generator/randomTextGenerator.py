...
class RandomWordGenerator:
    def __init__(self, vocab_path="./text_generator/vocab.txt"):
        self.p_upper = 0.5
        ...
        with open(vocab_path, "r", encoding="utf-8") as f:
            vocab = f.readlines()
        self.vocab = [word.strip().split()[0].replace('_',' ').replace('@','') for word in vocab]
...
class RandomSentenceWikiGenerator:
    def __init__(self, wiki_path='./text_generator/viwiki'):
        self.wiki_path = wiki_path
        self.file_list = os.listdir(wiki_path)
        self.max_len = 200

    def __get_len(self):
        return max(1, min(self.max_len, round(random.gauss(64, 32))))    
